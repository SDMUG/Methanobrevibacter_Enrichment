...
PCoA_enrichment <- plot_ordination(
  ...
  scale_color_manual(values = c("#6fa8dc", "#e06666")) +
  geom_point(aes(color = health_status), size = 3, show.legend = FALSE) +
  geom_point(colour = "grey90", size = 1.5) +
  geom_point(aes(shape = Strain), fill = "black") +
  stat_ellipse(aes(color = health_status, group = health_status), size = 1) +
